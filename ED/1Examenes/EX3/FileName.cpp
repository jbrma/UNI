 ...
 * --------------------------------------------------------------------
 *
 * Puedes añadir las funciones auxiliares que necesites, pero has de
...
bool cumple(const BinTree<int>& tree, int& hojas) {

    int hLeft = hojas, hRight = hojas;
    bool ok = true;
    
    if (tree.left().empty() && tree.right().empty()) { // si es una hoja
        hojas++;
        return true;
    }
    if (tree.left().empty()) {
    ...
    if (tree.right().empty()) {
        ...
        ok = ok && cumple(tree.right(), hRight);
    }

    ok = (ok && (tree.root() == hLeft - hRight));
    hojas = hLeft + hRight;
    return ok;
}
