...
#include <cassert>
   ...
   * Clase BinTree
   * --------------------------------------------------------------------
   * Implementa el TAD de los árboles binarios
   * --------------------------------------------------------------------
   */
...
/* --------------------------------------------------------------------
 * Implementa la función pedida a continuación.
 * --------------------------------------------------------------------
 *
 * Puedes añadir las funciones auxiliares que necesites.
 *
 *@ <answer>
 */
...
//
// COSTE: O(____________), donde n es el número de nodos del árbol de entrada.
// 


int bien(const BinTree<int>& tree, int& nivel) {

    int nivelAct = nivel;
    if (tree.empty())
        return -1;
    int nivelL = nivel + 1, nivelR = nivel + 1;
    int valorL = bien(tree.left(), nivelL);
    int valorR = bien(tree.right(), nivelR);

    if (valorL == -1 && valorR == -1) {
        return tree.root() % nivel == 0 ? tree.root() : -1;
    }
    else if (valorR == -1) {
        nivel = nivelL;
        return valorL;
        ...
        nivel = max(nivelL, nivelR);
        return nivelL >= nivelR ? valorL : valorR;
    }
...
void tratar_caso() {
    ...
    std::cout << nodo_bien_colocado(tree) << std::endl;
}
