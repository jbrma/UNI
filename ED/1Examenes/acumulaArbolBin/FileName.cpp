...
//
// ¡No olvides justificar el coste de la función!

int sumaRaices(BinTree<int>& arbol) {
    return arbol.left().root() + arbol.right().root();
}
...
BinTree<int> acumula(const BinTree<int>& arbol) {
    ...
    if (arbol.empty()) {
        ...
        a1 = acumula(arbol.left());
        a2 = acumula(arbol.right());
